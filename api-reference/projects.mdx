...
curl -X GET https://localhost:8001/api/v1/projects/list \
  -H "Authorization: Bearer YOUR_TOKEN"
```

```bash cURL (v2 - API Key)
curl -X GET https://localhost:8001/api/v2/projects/list \
...
def list_projects(token: str):
    ...
    url = "https://localhost:8001/api/v1/projects/list"
...
def list_projects_api_key(api_key: str, user_id: str):
    ...
    url = "https://localhost:8001/api/v2/projects/list"
    headers = {
        "x-api-key": api_key,
        "x-user-id": user_id
    }
...
async function listProjects(token: string): Promise<Project[]> {
  const response = await fetch('https://localhost:8001/api/v1/projects/list', {
    headers: {
...
async function listProjectsApiKey(apiKey: string, userId: string): Promise<Project[]> {
  const response = await fetch('https://localhost:8001/api/v2/projects/list', {
    headers: {
      'x-api-key': apiKey,
      'x-user-id': userId
    }
...
func listProjects(apiKey, userID string) ([]Project, error) {
    req, _ := http.NewRequest("GET", "https://localhost:8001/api/v2/projects/list", nil)
...
```bash cURL
curl -X DELETE "https://localhost:8001/api/v1/projects?project_id=proj_01HQXYZ789ABCDEF" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
def delete_project(project_id: str, token: str):
    """Delete a project"""
    url = "https://localhost:8001/api/v1/projects"
...
async function deleteProject(projectId: string, token: string) {
  const response = await fetch(
    `https://localhost:8001/api/v1/projects?project_id=${projectId}`,
    {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`
