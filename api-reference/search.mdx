...
curl -X POST https://localhost:8001/api/v1/search \
...
curl -X POST https://localhost:8001/api/v2/search \
...
def search_codebase(project_id: str, query: str, token: str):
    ...
    url = "https://localhost:8001/api/v1/search"
    headers = {
...
def search_codebase_api_key(project_id: str, query: str, api_key: str, user_id: str):
    ...
    url = "https://localhost:8001/api/v2/search"
    headers = {
        "x-api-key": api_key,
        "x-user-id": user_id,
        "Content-Type": "application/json"
...
async function searchCodebase(
  ...
  const response = await fetch('https://localhost:8001/api/v1/search', {
    method: 'POST',
    headers: {
...
async function searchCodebaseApiKey(
  ...
  const response = await fetch('https://localhost:8001/api/v2/search', {
    method: 'POST',
    headers: {
      'x-api-key': apiKey,
      'x-user-id': userId,
...
func searchCodebase(projectID, query, apiKey, userID string) (*SearchResponse, error) {
    ...
    req, _ := http.NewRequest(
        "POST",
        "https://localhost:8001/api/v2/search",
