...
function AgentSelector({
  ...
  useEffect(() => {
    fetch('https://production-api.potpie.ai/api/v2/list-available-agents/', {
...
|------|-------------------|
| Fix a bug | `debugging_agent` |
| Understand code | `codebase_qna_agent` |
| Write tests | `unit_test_agent`, `integration_test_agent` |
| Modify code | `code_changes_agent` |
| Code review | `codebase_qna_agent`, `code_changes_agent` |
| Architecture questions | `codebase_qna_agent` |
| Refactoring | `code_changes_agent` |
...
const response = await fetch(
  'https://production-api.potpie.ai/api/v2/conversations/',
  {
    ...
    body: JSON.stringify({
      ...
      agent_ids: [
        'codebase_qna_agent',   // For understanding existing code
        'code_changes_agent',   // For implementing changes
        'unit_test_agent'       // For writing tests
      ]
    })
  }
);
