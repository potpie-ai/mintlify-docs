...
response = requests.post(
    f'https://localhost:8001/api/v2/project/{project_id}/message/',
    headers={
...
```bash cURL
curl -X POST \
  "https://localhost:8001/api/v2/project/${PROJECT_ID}/message/" \
...
```bash
curl -X POST \
  "https://localhost:8001/api/v2/project/${PROJECT_ID}/message/?hidden=false" \
  -H 'x-api-key: YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
