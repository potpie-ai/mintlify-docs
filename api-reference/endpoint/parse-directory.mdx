...
response = requests.post(
    'https://localhost:8001/api/v2/parse',
...
while True:
    status = requests.get(
        f'https://localhost:8001/api/v2/parsing-status/{project_id}',
        headers={'x-api-key': 'YOUR_API_KEY'}
    ).json()
...
while (true) {
  const statusRes = await fetch(
    `https://localhost:8001/api/v2/parsing-status/${project_id}`,
    {
      headers: { 'x-api-key': 'YOUR_API_KEY' }
    }
  );
...
# 2. Monitor status
curl -X GET \
  'https://localhost:8001/api/v2/parsing-status/proj_123' \
  -H 'x-api-key: YOUR_API_KEY'
