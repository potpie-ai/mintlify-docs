...
const response = await fetch('https://locahost:8001/api/v1/projects/list', {
  headers: {
...
<Steps>
  <Step title="Parse Your Repository">
    Use the <a href="/api-reference/endpoint/parse-directory" className="mode-link">Parse Directory</a> endpoint to analyze your codebase and build the knowledge graph
  </Step>
  <Step title="Monitor Parsing">
    Poll the <a href="/api-reference/endpoint/get-parsing-status" className="mode-link">Get Parsing Status</a> endpoint until parsing is complete
  </Step>
  <Step title="Select an Agent">
    Use <a href="/api-reference/endpoint/list-agents" className="mode-link">List Agents</a> to see available AI agents and choose the right one for your task
  </Step>
  <Step title="Create Conversation">
    Start a conversation with <a href="/api-reference/endpoint/create-conversation" className="mode-link">Create Conversation</a> by providing project IDs and agent IDs
  </Step>
  <Step title="Interact">
    Send messages using <a href="/api-reference/endpoint/post-message" className="mode-link">Post Message</a> and receive AI-powered responses about your codebase
  </Step>
</Steps>
...
class PotpieClient {
  ...
  constructor(config: PotpieConfig) {
    ...
    this.baseUrl = config.baseUrl || 'https://locahost:8001';
  }
...
class PotpieClient:
    def __init__(self, api_key: str, base_url: str = "https://locahost:8001"):
