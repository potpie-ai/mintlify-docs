...
# Simple status check
curl -X GET \
  'https://localhost:8001/api/v1/parsing-status/proj_456' \
...
```typescript TypeScript
const response = await fetch('https://localhost:8001/api/v1/parsing-status', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer <token>`,
...
response = requests.post(
    'https://localhost:8001/api/v1/parsing-status',
    headers={
        'Authorization': 'Bearer <token>',
        'Content-Type': 'application/json'
    },
