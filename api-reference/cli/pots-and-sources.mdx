...
The diagram below shows how the three different ways context enters the graph relate to each other:

```mermaid
flowchart LR
  sourceAdd["potpie source add repo ."]
  resolveCmd["potpie resolve / graph read"]
  linearIngest["potpie pot linear-team ingest"]
  jiraIngest["potpie pot jira-project ingest"]
  metadata["source metadata stored\n(no graph write)"]
  onDemand["on-demand context pull\nby agent or developer"]
  queuedExternal["queued external-system\ningestion into graph"]

  sourceAdd --> metadata
  resolveCmd --> onDemand
  linearIngest --> queuedExternal
  jiraIngest --> queuedExternal
```

<Note>
`source add` and `ingest` are separate actions by design. You register a repository once; context is pulled when agents or developers actually need it.
</Note>
...
### Linear team ingestion

```bash
potpie pot linear-team ingest TEAM
potpie pot linear-team diff-sync TEAM
```

| Command | Purpose |
| --- | --- |
| `ingest` | Queue a one-shot ingestion for a Linear team's recent graph history. |
...
| Option | Description |
| --- | --- |
| `--pot` | Target pot, defaults to the active pot. |
| `--count` | Soft per-kind item limit, default `120`. |

### Jira project ingestion

```bash
potpie pot jira-project ingest PROJECT
potpie pot jira-project diff-sync PROJECT
```

| Command | Purpose |
| --- | --- |
| `ingest` | Queue one-shot Jira project ingestion into a pot. |
