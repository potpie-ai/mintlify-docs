...
<Card title="unit_test_agent" icon="flask">
  **Best for:** Generating unit tests for functions and methods
  ...
  **Example Questions:**
  - "Generate unit tests for the `authenticate_user` function"
  - "Create tests for the PaymentService class"
  - "What edge cases should I test for this function?"
</Card>

### Integration Test Agent

<Card title="integration_test_agent" icon="plug">
  **Best for:** Generating integration tests (Python, JavaScript, TypeScript)
  ...
  **Example Questions:**
  ...
  - "Create tests for the payment API endpoints"
  - "Write integration tests for the auth middleware"
</Card>

### Code Generation Agent

<Card title="code_gen_agent" icon="code">
...
| Writing tests | `unit_test_agent` or `integration_test_agent` | `code_gen_agent` |
| Adding features | `code_gen_agent` | `codebase_qna_agent` |
...
# Usage - Combine QnA and debugging agents
conv = create_multi_agent_conversation(
    "proj_01HQXYZ789ABCDEF",
    ["codebase_qna_agent", "debugging_agent"],
...
2. **unit_test_agent** - Writing tests
...
FEATURE_WORKFLOW = [
    ...
    "code_gen_agent",           # Implementation
    "unit_test_agent"           # Testing
]
...
DEBUG_WORKFLOW = [
    ...
    "unit_test_agent"           # Prevent regression
]
...
REVIEW_WORKFLOW = [
    ...
    "unit_test_agent"           # Verify coverage
]
```
...
<CardGroup cols={2}>
  <Card title="Single Purpose" icon="crosshairs">
    Use one agent per conversation for focused results
  </Card>
  ...
  <Card title="Provide Context" icon="code">
    Reference code nodes for accurate answers
  </Card>
</CardGroup>
