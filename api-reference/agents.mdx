...
curl -X GET "https://localhost:8001/api/v1/list-available-agents/?list_system_agents=true" \
  -H "Authorization: Bearer YOUR_TOKEN"
```

```bash cURL (v2 - API Key)
curl -X GET https://localhost:8001/api/v2/list-available-agents \
...
def list_agents(token: str, include_system: bool = True):
    ...
    url = "https://localhost:8001/api/v1/list-available-agents/"
...
def list_agents_api_key(api_key: str, user_id: str):
    ...
    url = "https://localhost:8001/api/v2/list-available-agents"
    headers = {
        "x-api-key": api_key,
        "x-user-id": user_id
    }
...
async function listAgents(
  ...
  const response = await fetch(
    `https://localhost:8001/api/v1/list-available-agents/?list_system_agents=${includeSystem}`,
    {
      headers: {
...
async function listAgentsApiKey(
  ...
  const response = await fetch(
    'https://localhost:8001/api/v2/list-available-agents',
    {
      headers: {
        'x-api-key': apiKey,
        'x-user-id': userId
...
1. **codebase_qna_agent** - Understanding code structure and functionality
2. **debugging_agent** - Identifying and fixing bugs
3. **code_generation_agent** - Generating new features and boilerplate code
