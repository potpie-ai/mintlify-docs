...
<Steps>
  ...
  <Step title="Send Your Question">
    ...
    const messageResponse = await fetch(
      `https://locahost:8001/api/v1/conversations/${conversationId}/message/?stream=false`,
      {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer <token>'
    ...
    curl -X POST "https://locahost:8001/api/v1/conversations/{conversation_id}/message/?stream=false" \
      -H "Authorization: Bearer <token>" \
...
curl -X POST https://locahost:8001/api/v1/conversations/{conversation_id}/message/ \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "content": "Where is authentication implemented?"
