...
**Ask** maps every dependent across the [context graph](/concepts/context-engine) using the **Node Neighbors** tool, organized by dependency type, so the full impact surfaces before touching a single line.
