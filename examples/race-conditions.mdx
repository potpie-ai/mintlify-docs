...
**Debug** walks the [context graph](/concepts/context-engine) through every concurrent code path involved in the failure, identifies the exact timing window where state is shared unsafely, and returns a targeted fix pinpointing the precise file and line that needs a guard.
