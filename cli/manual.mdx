...
- [CLI Reference](/cli/reference) — Command families and install flow.
- [Setup & Lifecycle](/cli/setup-and-lifecycle) — Setup, status, doctor, and config in depth.
- [Context Commands](/cli/context-commands) — `resolve`, `search`, and `record` in depth.
- [Integrations & Auth](/cli/integrations-and-auth) — Provider login flows.
- [Pots & Sources](/cli/pots-and-sources) — Workspace routing and source registration.
- [Graph & Operations](/cli/graph-and-operations) — Graph workbench and operational commands.
