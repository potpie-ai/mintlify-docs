...
2. At least one <a href="/concepts/parsing" className="mode-link">parsed</a> project with `ready` status
3. The PotpieAI app for Slack installed in the workspace

## Installation

1. [Install the PotpieAI app for Slack](https://potpie-slack-1060968446103.asia-south1.run.app/slack/install)
...
3. Enter a PotpieAI API token. Follow the [API access documentation](https://docs.potpie.ai/agents/api-access) to generate one.
4. Click **Submit**
...
Review the [Privacy Policy](https://potpie.ai/privacy-policy) for details on data collection and protection.
...
- The AI's understanding reflects the <a href="/concepts/parsing" className="mode-link">parsed</a> codebase and may not match the current state of the code
