...
<Note>
  **Looking for something specific?**
  - <a href="/agents/tools-reference" className="mode-link">Tools Reference</a> - Complete tool documentation
  - <a href="/agents/introduction#agent-selection-guide" className="mode-link">Choose an Agent</a> - Agent selection guide
</Note>
...
The Ask Agent operates through the <a href="/api-reference/endpoint/create-conversation" className="mode-link">Conversations API</a>. You create a conversation with the agent, then send messages. The agent processes questions using 19 specialized tools and returns structured markdown responses with code citations.
...
All agents return standard error responses. See the <a href="/agents/error-reference" className="mode-link">Error Reference</a> for complete documentation.
...
- <a href="/build-flow/build-a-feature" className="mode-link">Build a Feature</a> - Generates and manages code modifications. The Ask Agent also powers the clarifying questions in the Code Agent workflow.
