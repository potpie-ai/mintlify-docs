...
<Steps>
  <Step title="Sign in to the dashboard">
    Open [app.potpie.ai](https://app.potpie.ai) and sign in.
...
<Info>
  The `/api/v2` endpoints use **API key** authentication. For **Firebase Bearer token** authentication with `/api/v1` endpoints, see the agent-specific documentation.
</Info>
...
<Steps>
  ...
  <Step title="Send a message">
    ...
    **Response:**
    ```json
    {
      "response": "Markdown-formatted answer with citations",
      "tool_calls": [],
